...
class SetupLifecycleTest(unittest.TestCase):
    ...
    def _assert_typescript_templates(self) -> None:
        biome = (self.workspace / ".bazel_devtools/biome.json").read_text()
        self.assertIn('"preset": "recommended"', biome)
        ...
        self.assertNotIn('"jsx"', tsconfig)
        self.assertIn(
            '"extends": "./.bazel_devtools/tsconfig.json"',
            (self.workspace / "tsconfig.user.json").read_text(),
        )
        editor_tsconfig = (self.workspace / "tsconfig.json").read_text()
        self.assertTrue(editor_tsconfig.startswith("// Generated by bazel_devtools ide-sync."))
        self.assertIn('"files": []', editor_tsconfig)
    ...
    def test_default_templates_bootstrap_a_structurally_valid_repository(self) -> None:
        ...
        self._assert_typescript_templates()
        self.assertIn(
    ...
    def test_typescript_setup_generates_the_root_config_provider_graph(self) -> None:
        ...
        self.assertIn('src = "tsconfig.user.json"', root_build)
...
""",
    ...
    def test_brownfield_typescript_policy_requires_managed_inheritance(self) -> None:
        ...
        (self.workspace / "tsconfig.user.json").write_text("{}\n", encoding="utf-8")
        ...
        self.assertTrue(any("generated editor metadata" in issue for issue in result.conflicts))

    def test_brownfield_typescript_policy_accepts_jsonc_inheritance(self) -> None:
        templates = templates_for_languages(("typescript",))
        by_path = {template.path: template for template in templates}
        (self.workspace / "biome.json").write_text(
            """{
  // Native JSONC comments are valid here.
...
""",
        ...
        (self.workspace / "tsconfig.user.json").write_text(
            """{
  /* Application options remain user-owned. */
  "extends": "./.bazel_devtools/tsconfig.json",
}
""",
            encoding="utf-8",
        ...
        (self.workspace / "tsconfig.json").write_text(
            by_path["tsconfig.json"].content,
            encoding="utf-8",
        )

        result = plan_initialize(self.workspace, templates)

        self.assertEqual([], result.conflicts)

    def test_enabling_typescript_rejects_a_legacy_root_config_before_writes(self) -> None:
        python_templates = templates_for_languages(("python",))
        initialize(self.workspace, python_templates, languages=("python",))
    ...
    def test_brownfield_bazel_graph_collisions_require_explicit_migration(self) -> None:
        ...
        (tools / "format_driver.py").write_text("custom = True\n", encoding="utf-8")
        ...
        self.assertTrue(any("format_driver.py" in issue for issue in result.conflicts))
