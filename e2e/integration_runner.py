...
def _check_typescript_config_targets(workspace: Path, *, expected: bool) -> None:
    ...
    has_root_tsconfig = (
        ...
        and 'src = "tsconfig.user.json"' in root_build
    )
...
def _check_typescript_editor_metadata(workspace: Path) -> None:
    tsconfig_content = (workspace / "tsconfig.json").read_text(encoding="utf-8")
    marker, separator, json_content = tsconfig_content.partition("\n")
    if not separator or not marker.startswith("// Generated by bazel_devtools ide-sync."):
        _fail("tsconfig.json is not marked as generated editor metadata")
    decoded: object = json.loads(json_content)  # pyright: ignore[reportAny]
    tsconfig = _object(decoded, "tsconfig.json")
    if tsconfig.get("extends") != "./tsconfig.user.json":
        _fail("tsconfig.json does not extend the user-owned TypeScript config")
    typescript_files = _array(tsconfig.get("files"), "tsconfig.json files")
    if typescript_files != [
        "typescript/greeting.ts",
        "typescript/greeting_view.tsx",
    ]:
        _fail("tsconfig.json does not contain the exact Bazel-owned editor source set")
    tsconfig_user = _object(
        _json(workspace / "tsconfig.user.json"),
        "tsconfig.user.json",
    )
    if tsconfig_user.get("extends") != "./.bazel_devtools/tsconfig.json":
        _fail("tsconfig.user.json does not extend the managed diagnostics baseline")
...
""",
    ...
    def check_ide_metadata(self) -> None:
        unowned_typescript = self.workspace / "not_owned_by_bazel.ts"
        unowned_typescript.write_text(
            'export const notOwned = "not in the editor project";\n',
            encoding="utf-8",
        )
        ...
        _check_typescript_editor_metadata(self.workspace)
